...
router.get("/profile", verifyToken, async (req, res) => {
    try {
        ...
        const profilePhotoUrl = profile.profilePhotoId
            ? `/api/adminsetting/profile/photo/${profile.profilePhotoId}`
...
/* ✅ Fetch Admin Name */
router.get("/admin-name", verifyToken, async (req, res) => {
    try {
        const admin = await User.findOne({ email: req.adminEmail }).lean();

        if (!admin || !admin.name) {
            console.error("❌ Admin profile not found for email:", req.adminEmail);
            return res.status(404).json({ error: "Admin profile not found" });
        }

        console.log("✅ Fetched Admin Name for:", req.adminEmail);
        res.json({ adminName: admin.name });
    } catch (err) {
        console.error("❌ Error fetching admin name:", err);
        res.status(500).json({ error: "Failed to fetch admin name" });
    }
});

/* ✅ Update Profile (NO TOKEN REQUIRED) */
router.post("/profile", async (req, res) => {
    try {
        const { email, name, gender, profilePhotoId } = req.body;
        if (!email) return res.status(400).json({ error: "Email is required" });

        const updatedProfile = await User.findOneAndUpdate(
            { email },
            { name, gender, profilePhotoId },
...
router.post("/change-password", verifyToken, async (req, res) => {
    try {
        ...
        // ✅ Find the admin user & explicitly select password
        ...
        if (!admin.password) {
            return res.status(400).json({ error: "No password set for this admin" });
        }
