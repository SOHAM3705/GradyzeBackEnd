...
router.post("/add", async (req, res) => {
    try {
        ...
        if (!name || !email || !department || !subjects) {
        ...
        const newTeacher = new Teacher({ name, email, password: hashedPassword, department, subjects });
...
router.post("/login", async (req, res) => {
    try {
        ...
        res.status(200).json({
            ...
            token,
            teacher: { name: teacher.name, email: teacher.email, department: teacher.department, subjects: teacher.subjects },
        });
    } catch (error) {
...
router.get("/dashboard", authMiddleware, async (req, res) => {
    try {
        ...
        let teacher = await Teacher.findOne({ email });
        if (!teacher) return res.status(404).json({ message: "Teacher not found" });
        res.status(200).json({ name: teacher.name, email: teacher.email, department: teacher.department, subjects: teacher.subjects });
    } catch (error) {
